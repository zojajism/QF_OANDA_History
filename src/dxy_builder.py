...
class _DXYBuilder:
    ...
    async def on_source_candle(self, candle: dict) -> None:
        ...
        # TODO: insert DXY candle into database (step 2)
        logger.info(json.dumps({"EventType": "DXY_Candle", **{k: str(v) for k, v in dxy_candle.items()}}))
        ctx.created = True
        self._maybe_cleanup()
...
async def on_source_candle(candle: dict) -> None:
    await _BUILDER.on_source_candle(candle)
